...
# ============================================================================
# AUDIT REPORT MANAGEMENT ENDPOINTS (Standalone Audit Scanner Integration)
# ============================================================================

@app.post("/audit/upload-report")
async def upload_audit_report(file: UploadFile = File(...)):
    """
    Upload audit report PDF from standalone CIS-Audit-Scanner.exe
    
    Accepts PDF reports generated by the audit scanner and provides
    ...
    try:
        ...
        if not file.filename.endswith('.pdf'):
            raise HTTPException(
                status_code=400, 
                detail="Invalid file type. Please upload a PDF report from CIS-Audit-Scanner.exe"
            )
        
        # Create audit_data directory if not exists
        ...
        # Save uploaded PDF
        pdf_filename = f"audit_report_{timestamp}_{report_id}.pdf"
        pdf_path = audit_dir / pdf_filename
        
        async with aiofiles.open(pdf_path, 'wb') as f:
            content = await file.read()
            await f.write(content)
        
        # Extract text from PDF for analysis (basic metadata)
        try:
            from PyPDF2 import PdfReader
            reader = PdfReader(pdf_path)
            text_content = ""
            for page in reader.pages[:3]:  # Read first 3 pages for metadata
                text_content += page.extract_text()
            
            # Try to extract basic info from PDF text
            computer_name = "Unknown"
            compliance_score = 0.0
            scan_date = timestamp
            
            # Simple extraction (the PDF has structured text)
            if "Computer:" in text_content:
                computer_name = text_content.split("Computer:")[1].split("\n")[0].strip()
            if "Overall Compliance Score:" in text_content or "compliance score:" in text_content.lower():
                import re
                score_match = re.search(r'(\d+\.?\d*)%', text_content)
                if score_match:
                    compliance_score = float(score_match.group(1))
            if "Scan Date:" in text_content:
                scan_date = text_content.split("Scan Date:")[1].split("\n")[0].strip()
        ...
        metadata_path = audit_dir / f"audit_metadata_{timestamp}_{report_id}.json"
        ...
        return {
            ...
            "message": f"Audit report uploaded successfully. Compliance score: {compliance_score}%",
            "next_steps": [
                "Review the compliance score and findings",
                "Check critical findings in the report",
                "Download remediation script if needed",
                "Apply fixes and re-run audit scanner"
            ]
        }
...
async def list_audit_reports():
    """
    List all uploaded audit reports
    
    Returns list of audit reports with metadata
    """
    try:
        audit_dir = Path("audit_data")
        ...
        for metadata_file in audit_dir.glob("audit_metadata_*.json"):
            try:
                with open(metadata_file, 'r') as f:
                    metadata = json.load(f)
                
                # Check if PDF still exists
                pdf_path = audit_dir / metadata.get('pdf_filename', '')
                pdf_exists = pdf_path.exists() if metadata.get('pdf_filename') else False
        ...
        print(f"Error listing audit reports: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/audit/report/{report_id}")
async def get_audit_report_details(report_id: str):
    """
    Get detailed information about a specific audit report
    
    Returns metadata and allows PDF download
    """
    try:
        audit_dir = Path("audit_data")
        
        # Find metadata file for this report
        metadata_files = list(audit_dir.glob(f"audit_metadata_*_{report_id}.json"))
        ...
        return {
            ...
            "report": {
                **metadata,
                ...
                "grade": grade,
                "status": status,
                "download_url": f"/audit/download/{report_id}" if pdf_exists else None
            }
        }
        ...
        print(f"Error getting audit report details: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def download_audit_report(report_id: str):
    """
    Download the PDF report for a specific audit
    """
    try:
        audit_dir = Path("audit_data")
        
        # Find metadata to get PDF filename
        metadata_files = list(audit_dir.glob(f"audit_metadata_*_{report_id}.json"))
        ...
        pdf_filename = metadata.get('pdf_filename')
        if not pdf_filename:
            raise HTTPException(status_code=404, detail="PDF filename not found in metadata")
        ...
        print(f"Error downloading audit report: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.delete("/audit/report/{report_id}")
async def delete_audit_report(report_id: str):
    """
    Delete an audit report and its metadata
    """
    try:
        audit_dir = Path("audit_data")
        ...
        print(f"Error deleting audit report: {e}")
        raise HTTPException(status_code=500, detail=str(e))
